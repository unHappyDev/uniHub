...
export default function AlunoBody() {
  ...
  const alunoQuickAccessCards = [
    {
      href: "/aluno/notas",
      icon: <FileText className="w-6 h-6" />,
      title: "Minhas Notas",
      desc: "Consultar notas e frequência",
  ...
  return (
    <div className="p-4 bg-[#141414] text-white min-h-screen">
      <div className="flex flex-col gap-8 p-2 sm:p-4">
        ...
        <div className="md:hidden grid grid-cols-3 gap-2 mb-4 w-full">
          {alunoQuickAccessCards.map((item, idx) => (
            <Link key={idx} href={item.href} className="w-full">
              <div
                className="bg-[#1a1a1dc3] border border-orange-400/20 p-3 rounded-xl 
                      flex flex-col items-center justify-center 
                      hover:border-orange-400/50 transition-all w-full h-24"
              >
          ...
          <div className="overflow-x-auto no-scrollbar pb-2">
            <div className="flex gap-4 min-w-max">
              {diasDaSemana.map((dia) => (
                <div
                  key={dia.key}
                  className="bg-[#1a1a1d] border border-orange-400/30 rounded-xl p-4 w-52 flex-shrink-0 hover:border-orange-500/70 transition-all"
                >
                  ...
                  horariosPorDia[dia.key].length > 0 ? (
                    horariosPorDia[dia.key].map((h, idx) => (
                      <div
                        key={idx}
                        className="bg-[#1a1a1d] p-3 rounded-lg border border-orange-400/20 mb-3"
                      >
          ...
          <div>
            ...
            {loadingPosts ? (
              ...
              posts.slice(0, 2).map((post) => (
                <Link key={post.postId} href="/aluno/avisos">
        ...
        <div className="hidden md:block">
          ...
          {loadingHorarios ? (
            ...
            <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
              {diasDaSemana.map((dia) => (
                <Card
                  key={dia.key}
                  className="bg-[#1a1a1dc3] border border-orange-400/20 rounded-xl p-4 hover:border-orange-500/70 transition-all"
                >
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    horariosPorDia[dia.key].length > 0 ? (
                      horariosPorDia[dia.key].map((h, idx) => (
                        <div
                          key={idx}
                          className="bg-[#1a1a1d] p-2 rounded-lg border border-orange-400/20 hover:border-orange-500/50 transition-all"
                        >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 bg-[#1a1a1dc3] border border-orange-400/20 rounded-xl p-6">
              <div className="flex items-center gap-2 mb-4">
              ...
              {loadingPosts ? (
                ...
                <p className="text-gray-400 text-center">Nenhum aviso</p>
              ) : (
            ...
            <div className="flex flex-col gap-4">
              {alunoQuickAccessCards.map((item, idx) => (
                <Link key={idx} href={item.href}>
                  <Card className="bg-[#1a1a1dc3] border border-orange-400/20 hover:border-orange-400/60 cursor-pointer rounded-2xl p-4 flex items-center gap-4">
                    <div
